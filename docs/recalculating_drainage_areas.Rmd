...
flowlines_trim <- flowlines %>% filter(comid %in% lshasta_clean$comid) %>% 
        select(id:pathlength, uphydroseq:dnhydroseq, areasqkm:divdasqkm, hwnodesqkm, lakefract:surfarea)

mapview(catchment_clean, zcol="areasqkm", layer.name="Catchments Area") +
        mapview(flowlines_trim, zcol="dnhydroseq", lwd=2)
