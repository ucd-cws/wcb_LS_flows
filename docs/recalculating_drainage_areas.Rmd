...
This map shows the original streamlines (light blue thin lines) vs. the cleaned streamlines (dark blue thick lines).
...
catchment_clean <- catchment_clean %>% 
...
flowlines_trim2 <- flowlines_trim %>% 
    filter(hydroseq <=10035874 | comid==3917230) %>% 
    left_join(., st_drop_geometry(catchment_clean[,c("featureid","areasqkm", "areasqkm_rev")]), by=c("comid"="featureid"))

# quick preview maps
mapview(catchment_clean, alpha.regions=0.1, color="gray", legend=F) + mapview(flowlines_trim2, zcol="comid", legend=F)
...
# everything is the same upstream of hydroseq 10035874
flowlines_trim2 %>% 
        arrange(desc(comid)) %>% View()
        
# can use fromnode and tonode to calculate drainage
flowlines_trim2 %>% 
        arrange(desc(comid)) %>% View()
...
Use the timing from upstream segment (3917948) and propagate downstream.

Does appear they are different.
